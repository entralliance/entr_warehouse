...
{% macro flag_min_max(column_name, min, max, alias, where='1=1', inclusive=false) %}
    case
        when {{min_max_condition(column_name, min, max, where, inclusive)}} then 1
        else 0
...
{% macro stale_condition(column_name, datetime_column, partition_by_cols, exclude_values=[], lag_length=1) %}
    {% if partition_by_columns is defined %} 
        {{ column_name }} = lag( {{ column_name }}, {{ lag_length }} ) over ( partition by {{ partition_by_columns}} order by {{datetime_column}})
    {%- else -%}
        {{ column_name }} = lag( {{ column_name }}, {{ lag_length }} ) over ( order by {{datetime_column}})
    {% endif %}
    {% if exclude_values|length > 0 %} 
        and {{ column_name }} not in ({{ jinja_list_to_sql(exclude_values) }})
    {% endif %}
{% endmacro %}

{% macro flag_stale(column_name, datetime_column, partition_by_cols, exclude_values=[], lag_length=1) %}
    case
        when {{stale_condition(column_name, datetime_column, partition_by_cols, exclude_values, lag_length)}} then 1
        else 0
    end as {% if alias is defined %} {{alias}} {% else %} {{ 'flag_stale_' + column_name}} {% endif %}
{% endmacro %}
